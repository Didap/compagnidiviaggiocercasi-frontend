...
<template>
  <section class="relative w-full h-[100dvh] overflow-hidden bg-bg-primary flex items-center justify-center">
    <!-- Foreground Animated Elements - Airplanes & Globe -->
    <div class="absolute inset-0 pointer-events-none select-none overflow-hidden z-20">
      <!-- Globe (Top Left relative to text) -->
      <img
    ...
    <!-- Background Animated Elements - Blobs -->
    ...
    <div class="container mx-auto px-6 relative z-30 flex flex-col items-center justify-center h-full text-center pt-10">
      <div class="max-w-4xl space-y-6">
        
        <!-- Badge / Eyebrow Text -->
        <span class="inline-block py-1 px-3 rounded-full bg-secondary/10 text-secondary font-semibold text-sm tracking-widest uppercase mb-4">
        ...
        <h1 class="text-5xl md:text-7xl lg:text-8xl font-black text-primary leading-[0.9] tracking-tight">
          Non siamo <br/>
          <span class="text-secondary/80">il solito</span> <br/>
          tour operator.
        </h1>

        <!-- Subtext -->
        <p class="text-xl md:text-2xl text-gray-600 font-medium max-w-2xl mx-auto leading-relaxed">
          Connettiamo persone, culture e storie attraverso la nostra più grande passione: <span class="bg-primary/10 text-primary px-2 py-1 rounded-md font-bold">VIAGGIARE</span>.
        </p>

        <!-- CTAs -->
        <div class="flex flex-col sm:flex-row justify-center gap-4 pt-8">
          <Button size="lg" class="h-14 px-10 text-lg rounded-full shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 transition-all hover:-translate-y-1">
            Scopri i Viaggi
          </Button>
          <Button variant="outline" size="lg" class="h-14 px-10 text-lg rounded-full border-2 border-secondary/30 text-secondary hover:bg-secondary hover:text-white transition-all">
            Chi Siamo
          </Button>
        </div>
      </div>
...
<style scoped>
/* Custom utility for nicer font rendering on large headings */
h1 {
